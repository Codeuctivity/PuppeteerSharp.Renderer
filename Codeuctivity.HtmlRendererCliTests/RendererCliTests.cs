...
namespace Codeuctivity.HtmlRendererCliTests
{
    public class RendererCliTests
    {
        ...
        private static string DotnetPublishFolderProfileWindows(string projectName)
        {
            ...
            var expectedBinaryPath = Path.Combine(absolutePath, $"bin/Release/net10.0/publish/FolderProfileWindows/{projectName}.exe");
            ...
            using var process = new Process
            {
                ...
                {
                    FileName = "dotnet",
                    Arguments = $"publish {absolutePath} --configuration Release -f net10.0 -p:PublishProfile=FolderProfileWindows",
                    RedirectStandardOutput = true,
                    RedirectStandardError = true,
                    UseShellExecute = false,
                    CreateNoWindow = true,
